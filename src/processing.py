...
from src.settings import check_size, debug_windows, ignore_list, window_check_title
...
def get_game_window():
    ...
    for window in windows:
        if window_check_title.lower() in window.title.lower() and ignore_list_check(
            window.title.lower(), ignore_list
        ):  # check for window with keyword & not browser
            game_window = window
            break
    ...
    if not game_window.isActive:
        try:
            ...
            print(
                "The program will attempt to screenshot anyway--it will likely not work as intended."
            )
...
def get_game_window_center(game_window) -> tuple:
    return (
        game_window.left + game_window.width // 2,
...
def get_window_screenshot(check_length):
    ...
    if check_length != 0:
        ...
        left = max(
            game_window.left, center_x - (check_length // 2)
        )  # subtract half check length since square is centered
        top = max(game_window.top, center_y - (check_length // 2))

        check_w, check_h = (check_length, check_length)

        if (
            left + check_length > game_window.left + game_window.width
        ):  # ensure it doesn't check out of game window
...
def detect_exclamation(contour_check_area: int) -> bool:
    try:
        ...
        mask1 = cv2.inRange(
            screenshot, target_colors1["lower"], target_colors1["higher"]
        )
        mask2 = cv2.inRange(
            screenshot, target_colors2["lower"], target_colors2["higher"]
        )
...
def found_text_in_image() -> bool:
    try:
        ...
        extracted_text = re.sub(
            r"[^a-zA-Z0-9]", " ", extracted_text
        )  # remove special characters that sometimes artifact
