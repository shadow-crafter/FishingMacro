...
class Macro:
    ...
    def click(cls):
        try:
            ...
            time.sleep(
                click_interval + (random.random() / 1000)
            )  # variance for bot detection
    ...
    class SearchingMode(MacroState):
        ...
        def execute(self):
            while time.time() - Macro.last_clicked_time < alarm_time:
                ...
                if (
                    detect_exclamation(contour_area_threshold)
                    and time.time() - Macro.last_clicked_time >= exclamation_wait_time
                ):  # sometimes exclamation lingers
